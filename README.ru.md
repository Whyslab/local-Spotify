...
Принимаются только ссылки на `youtube.com`, `m.youtube.com`, `music.youtube.com` и `youtu.be`, включая Shorts, эфиры и встраивание (`/shorts/ID`, `/live/ID`, `/embed/ID`); разные формы одной и той же ссылки на один и тот же ролик приводятся к единому каноническому виду и не создают дубликат задачи. Повторная отправка ссылки с уже завершившейся или выполняющейся задачей игнорируется; ссылку с задачей в статусе `error` можно переотправить, чтобы поставить её в очередь заново.
