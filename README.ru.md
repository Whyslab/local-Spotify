...
* **Подборки — это файлы.** Каждая подборка лежит в корне фонотеки как `.m3u` и правится перезаписью файла. Перестановку через API Navidrome молча игнорирует, если подборка привязана к файлу; перезапись работает, и он перечитывает её примерно за десять секунд.
...
| `GET`  | `/api/plays/stats` | Доля пропусков и размер журнала |
