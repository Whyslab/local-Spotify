...
def app(tmp_path, monkeypatch):
    ...
    module = importlib.import_module("adder.app")
    monkeypatch.setattr(module, "DB_PATH", tmp_path / "tasks.db")
    ...
    monkeypatch.setattr(module, "check_disk_space", lambda: (True, 10_000))
    module.TMP_DIR.mkdir()
    module.LIBRARY.mkdir()
    module.PROCESSING_URLS.clear()
...
def youtube(app, monkeypatch, meta, audio=FIXTURE):
    ...
    monkeypatch.setattr(app, "yt_meta", lambda url: {"id": "abc123", **meta})

    def download(url, vid):
        target = app.TMP_DIR / f"{vid}.m4a"
    ...
    monkeypatch.setattr(app, "yt_download", download)


def deezer(app, monkeypatch, info):
    """Fake enrichment: `info` as a Deezer hit, or None for "not found"."""
    monkeypatch.setattr(app.enrich, "lookup", lambda artist, title: info)
...
def covers(app, monkeypatch, deezer_cover=None, fallback_cover=None):
    ...
    monkeypatch.setattr(app, "fetch_cover_url", by_url)
    monkeypatch.setattr(app, "fetch_cover", by_search)
    return requested


def run(app, url=URL):
    tid = app.db_exec("INSERT INTO tasks(url, status) VALUES(?, 'queued')", (url,)).lastrowid
    app.process(tid, url)
    return app.db_query("SELECT * FROM tasks WHERE id = ?", (tid,))[0]


def library_files(app):
    return sorted(p.relative_to(app.LIBRARY) for p in app.LIBRARY.rglob("*.m4a"))
...
def test_deezer_match_is_tagged_and_filed_for_navidrome(app, monkeypatch, caplog):
    ...
    tags = MP4(app.LIBRARY / "Daft Punk/Singles/Get Lucky.m4a").tags
    ...
    assert any("Added to library" in r.getMessage() for r in caplog.records)
...
def test_unknown_track_becomes_its_own_single_with_the_fallback_cover(app, monkeypatch):
    ...
    tags = MP4(app.LIBRARY / "Small Band/Singles/Rare Song.m4a").tags
...
def test_track_without_any_cover_is_still_added(app, monkeypatch):
    ...
    assert task["status"] == "done", task["error"]
    assert "covr" not in MP4(app.LIBRARY / "A/Singles/B.m4a").tags
...
def test_same_name_different_audio_is_kept_beside_the_first(app, monkeypatch):
    ...
    other = app.TMP_DIR.parent / "other.m4a"
...
def test_unavailable_video_fails_once_without_retrying(app, monkeypatch):
    ...
    monkeypatch.setattr(app, "MAX_RETRIES", 3)
    monkeypatch.setattr(app, "yt_meta", unavailable)
...
def test_rate_limit_is_retried_then_succeeds(app, monkeypatch):
    ...
    monkeypatch.setattr(app, "yt_meta", throttled_once)
    monkeypatch.setattr(app, "MAX_RETRIES", 2)
    monkeypatch.setattr(app.shutdown_event, "wait", lambda seconds: False)  # no real backoff
    deezer(app, monkeypatch, None)
    covers(app, monkeypatch)
...
def test_bot_check_error_tells_the_user_what_to_set(app, monkeypatch):
    ...
    monkeypatch.setattr(app, "yt_meta", bot_check)
...
def test_new_track_is_searchable_immediately(app, monkeypatch):
    app.library_index()  # warm the 60-second cache while the library is empty
    youtube(app, monkeypatch, {"title": "A - B", "uploader": "x"})
    deezer(app, monkeypatch, None)
    covers(app, monkeypatch)
    ...
    assert [row["title"] for row in app.library_index()] == ["B"]
