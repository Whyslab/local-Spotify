...
def app(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(module, "COOKIES_FROM_BROWSER", "")
...
def test_cookies_are_sent_with_the_metadata_request(app, monkeypatch):
    ...
    monkeypatch.setattr(app, "COOKIES_FROM_BROWSER", "firefox")
...
def test_download_extracts_m4a_into_tmp_and_returns_it(app, monkeypatch):
    def reply(cmd):
        (app.TMP_DIR / "abc123.m4a").write_bytes(b"audio")
    ...
    assert result == app.TMP_DIR / "abc123.m4a"
    cmd = fake.commands[0]
    assert cmd[cmd.index("--audio-format") + 1] == "m4a"
    assert cmd[cmd.index("-o") + 1] == str(app.TMP_DIR / "abc123.%(ext)s")
    assert "--no-playlist" in cmd


def test_download_accepts_another_extension_when_m4a_is_missing(app, monkeypatch):
    def reply(cmd):
        (app.TMP_DIR / "abc123.mp4").write_bytes(b"audio")
        return completed(cmd)

    monkeypatch.setattr(app, "run_yt_dlp", FakeYtDlp(reply))

    assert app.yt_download("u", "abc123") == app.TMP_DIR / "abc123.mp4"
...
def test_run_stops_the_process_on_shutdown(app):
    ...
    try:
        with pytest.raises(app.ShutdownRequested):
